"use client";
import Link from "next/link";
import { useState } from "react";
// Removed: import ModalVideo from "react-modal-video";

// 1. Reusable Tab Content Component
// This component now uses the passed title, description, and image source.
const TabContent = ({ isActive, id, title, description, imgSrc }) =>
  isActive && (
    <div className="fc-profit-single row " id={id}>
      {/* Thumb Section (Replaces Video) */}
      <div className="thumb thumb-lg col-lg-6">
        {/* Static Image */}
        <img className="" src={imgSrc} alt={`Image for ${title}`} />

        {/* Removed: Video Button Wrapper */}

        <img
          src="assets/images/profit-shape.png"
          alt="Decorative shape"
          className="base-img"
        />
      </div>

      {/* Content Group Section */}
      <div className="fc-profit-group align-items-end col-lg-6">
        {/* New: Custom Content based on your descriptions */}
        <div className="tab-description-block  px-4">
          <h5>{title}</h5>
          <p>{description}</p>
        </div>
      </div>
    </div>
  );

const ProfitOne = () => {
  const [active, setActive] = useState(1);
  // Removed: const [isOpen, setIsOpen] = useState(false);

  // Define content data for easy management
  const tabData = [
    {
      id: "profit-one",
      title: "Feeding the Helpless",
      description:
        "We run meal distribution drives for both homeless individuals and stray animals, ensuring they get healthy and nutritious food. With your help, we can reach more empty plates and turn hunger into hope.",
      imgSrc: "assets/images/home/serviceOne.png", // Placeholder image path, update as needed
    },
    {
      id: "profit-two",
      title: "Healing the Vulnerable",
      description:
        "Through our medical aid services, we provide timely treatment to those suffering on the streets. We treat injuries, chronic illnesses, and malnutrition, giving them a chance to recover and regain strength.",
      imgSrc: "assets/images/home/serviceTwo.png", // Placeholder image path, update as needed
    },
    {
      id: "profit-three",
      title: "Shelter and Care",
      description:
        "Our shelters are open to all—humans and animals alike. We provide a clean, safe, and caring environment where the abandoned can heal, rest, and be cared for until they are ready for a new start.",
      imgSrc: "assets/images/home/serviceThree.png", // Placeholder image path, update as needed
    },
  ];

  return (
    <>
      <section className="fc-profit pt-120 pb-120 bg-fc">
        <div className="container">
          {/* Header Row */}
          <div className="row gutter-30 align-items-center">
            <div className="col-12 col-md-8 col-xl-7">
              <div className="section__header mb-0">
                <span className="sub-title">Service</span>
                <h2 className="title-animation_inner mt-0 fw-7 hb">
                  Providing Help, Hope, and Healing
                </h2>
                <p>
                  We are committed to serving the homeless and helpless offering
                  essential services, ranging from food provision to medical
                  care, to restoring dignity to those in need.
                </p>
              </div>
            </div>
          </div>
          {/* Main Content Row */}
          <div className="row gutter-40 mt-60">
            {/* Tab Buttons Column */}
            <div className="col-12 col-lg-5 col-xl-5">
              <div className="fc-profit__tab-btns">
                {/* Tab Button 1: Fund Raised & Donations */}
                <button
                  onClick={() => setActive(1)}
                  className={`profit__tab-btn ${active === 1 ? "active" : ""}`}
                  data-target="#profit-one"
                  aria-label="fund raised & donations"
                  title="fund raised & donations"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={32}
                    height={31}
                    viewBox="0 0 32 31"
                    fill="none"
                  >
                    <path d="M31.5852 22.2394C30.3189 19.036 25.3417 13.457 22.9332 10.9036H23.0471C23.5648 10.9036 23.9844 10.5076 23.9844 10.0191C23.9844 9.5306 23.5648 9.13462 23.0471 9.13462H22.8914C24.3674 7.5712 26.5823 5.21724 27.2042 4.51327C28.4895 3.05826 28.0501 1.75654 27.2834 1.08395C26.1522 0.0917443 23.9656 3.21567e-05 21.9421 1.35625C20.4541 0.0137153 17.5128 0.0164284 16.0279 1.35625C14.0044 0.000150174 11.8178 0.0917443 10.6866 1.08395C9.91981 1.75654 9.4805 3.05826 10.7658 4.51327C11.3878 5.2173 13.6026 7.5712 15.0786 9.13462H14.9229C14.4053 9.13462 13.9856 9.53066 13.9856 10.0191C13.9856 10.5076 14.4053 10.9036 14.9229 10.9036H15.0368C13.7491 12.2688 12.5469 13.6027 11.2114 15.1979C11.0988 14.8287 10.7388 14.5583 10.3113 14.5583H0.937375C0.419688 14.5584 0 14.9544 0 15.4429V29.6508C0 30.1392 0.419688 30.5353 0.937375 30.5353H25.4144C29.9036 30.5353 33.1953 26.3128 31.5852 22.2394ZM11.9631 2.37959C12.4114 1.98626 13.9803 1.84365 15.5749 3.29624C16.0806 3.75675 16.9257 3.54672 17.1249 2.91046C17.2287 2.57882 17.9329 2.11944 18.985 2.11944C20.0371 2.11944 20.7413 2.57882 20.8451 2.91046C21.0441 3.54637 21.8892 3.7571 22.3951 3.29624C23.9898 1.84359 25.5584 1.9862 26.0069 2.37959C26.3213 2.65538 25.9897 3.12508 25.7641 3.38052C24.9726 4.27653 21.3754 8.08184 20.3791 9.13462H17.5908C16.5946 8.08184 12.9974 4.27653 12.2059 3.38052C11.9803 3.12508 11.6487 2.65538 11.9631 2.37959ZM6.71306 25.17H1.87481V23.4018H7.58075C7.16306 23.9148 6.86138 24.5158 6.71306 25.17ZM9.37394 21.6328H1.87481V19.8646H9.37394V21.6328ZM9.37394 16.3274V18.0956H1.87481V16.3274H9.37394ZM1.87481 26.939H6.69431C6.83225 27.6148 7.13244 28.2366 7.55613 28.7662H1.87481V26.939ZM8.48338 26.0988C8.48338 24.6279 9.7515 23.4313 11.3121 23.4313C12.8708 23.4313 14.1389 24.6279 14.1389 26.0988C14.1389 27.5696 12.8708 28.7662 11.3121 28.7662C9.74638 28.7662 8.48338 27.5654 8.48338 26.0988ZM25.4144 28.7662H15.0663C15.6606 28.0232 16.0138 27.0993 16.0138 26.0988C16.0138 23.6482 13.8898 21.6323 11.2487 21.6637V18.0617C13.7594 14.856 16.3041 12.221 17.5467 10.9036H20.4233L22.0344 12.6116C22.0988 12.68 28.4948 19.4835 29.8286 22.8577C30.9578 25.7143 28.6749 28.7662 25.4144 28.7662Z" />
                  </svg>{" "}
                  Feeding the Needy
                  <i className="fa-solid fa-arrow-right" />
                </button>

                {/* Tab Button 2: Medical Treatment & Help */}
                <button
                  onClick={() => setActive(2)}
                  className={`profit__tab-btn ${active === 2 ? "active" : ""}`}
                  data-target="#profit-two"
                  aria-label="medical treatment &  help"
                  title="medical treatment &  help"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={30}
                    height={29}
                    viewBox="0 0 30 29"
                    fill="none"
                  >
                    <path d="M17.8125 13.5237V11.7543C17.8125 11.5197 17.7137 11.2947 17.5379 11.1287C17.3621 10.9628 17.1236 10.8696 16.875 10.8696H13.125C12.8764 10.8696 12.6379 10.9628 12.4621 11.1287C12.2863 11.2947 12.1875 11.5197 12.1875 11.7543V13.5237H10.3125C10.0639 13.5237 9.8254 13.6169 9.64959 13.7828C9.47377 13.9487 9.375 14.1737 9.375 14.4084V17.9471C9.375 18.1817 9.47377 18.4067 9.64959 18.5727C9.8254 18.7386 10.0639 18.8318 10.3125 18.8318H12.1875V20.6011C12.1875 20.8358 12.2863 21.0608 12.4621 21.2267C12.6379 21.3926 12.8764 21.4858 13.125 21.4858H16.875C17.1236 21.4858 17.3621 21.3926 17.5379 21.2267C17.7137 21.0608 17.8125 20.8358 17.8125 20.6011V18.8318H19.6875C19.9361 18.8318 20.1746 18.7386 20.3504 18.5727C20.5262 18.4067 20.625 18.1817 20.625 17.9471V14.4084C20.625 14.1737 20.5262 13.9487 20.3504 13.7828C20.1746 13.6169 19.9361 13.5237 19.6875 13.5237H17.8125ZM18.75 17.0624H16.875C16.6264 17.0624 16.3879 17.1556 16.2121 17.3215C16.0363 17.4874 15.9375 17.7125 15.9375 17.9471V19.7165H14.0625V17.9471C14.0625 17.7125 13.9637 17.4874 13.7879 17.3215C13.6121 17.1556 13.3736 17.0624 13.125 17.0624H11.25V15.293H13.125C13.3736 15.293 13.6121 15.1998 13.7879 15.0339C13.9637 14.868 14.0625 14.643 14.0625 14.4084V12.639H15.9375V14.4084C15.9375 14.643 16.0363 14.868 16.2121 15.0339C16.3879 15.1998 16.6264 15.293 16.875 15.293H18.75V17.0624Z" />
                    <path d="M27.1875 7.33097H20.625V4.67692C20.625 4.44229 20.5262 4.21726 20.3504 4.05135C20.1746 3.88544 19.9361 3.79224 19.6875 3.79224H10.3125C10.0639 3.79224 9.8254 3.88544 9.64959 4.05135C9.47377 4.21726 9.375 4.44229 9.375 4.67692V7.33097H2.8125C2.56386 7.33097 2.3254 7.42418 2.14959 7.59009C1.97377 7.756 1.875 7.98102 1.875 8.21565V24.1399C1.875 24.3746 1.97377 24.5996 2.14959 24.7655C2.3254 24.9314 2.56386 25.0246 2.8125 25.0246H27.1875C27.4361 25.0246 27.6746 24.9314 27.8504 24.7655C28.0262 24.5996 28.125 24.3746 28.125 24.1399V8.21565C28.125 7.98102 28.0262 7.756 27.8504 7.59009C27.6746 7.42418 27.4361 7.33097 27.1875 7.33097ZM11.25 5.5616H18.75V7.33097H11.25V5.5616ZM26.25 23.2553H3.75V9.10033H26.25V23.2553Z" />
                  </svg>{" "}
                  medical treatment &amp; help{" "}
                  <i className="fa-solid fa-arrow-right" />
                </button>

                {/* Tab Button 3: Development Programs */}
                <button
                  onClick={() => setActive(3)}
                  className={`profit__tab-btn ${active === 3 ? "active" : ""}`}
                  data-target="#profit-three"
                  aria-label="development programs"
                  title="development programs"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={33}
                    height={32}
                    viewBox="0 0 33 32"
                    fill="none"
                  >
                    <g clipPath="url(#clip0_51_376)">
                      <path d="M16.6428 15.8756C14.8971 17.276 13.1729 18.1807 13.1004 18.2185C12.8095 18.3702 12.4562 18.3703 12.1652 18.2185C12.0927 18.1807 10.3684 17.276 8.6228 15.8756C6.15147 13.893 4.89844 11.9465 4.89844 10.0902C4.89844 6.16856 9.78643 4.08419 12.6328 6.76881C15.4843 4.07963 20.3672 6.17701 20.3672 10.0902C20.3671 11.9465 19.1141 13.893 16.6428 15.8756ZM16.3131 7.46212C15.1118 7.1709 14.0745 7.69969 13.4799 8.71998C13.1132 9.3493 12.1523 9.3493 11.7856 8.71998C11.1639 7.65304 10.1048 7.18282 8.95241 7.46212C7.89699 7.71794 6.83203 8.62972 6.83203 10.0902C6.83203 12.545 10.658 15.2371 12.6291 16.363C14.7982 15.1101 18.4335 12.4919 18.4335 10.0902C18.4335 8.62978 17.3686 7.71794 16.3131 7.46212Z" />
                      <path d="M12.6328 3.73507C12.0989 3.73507 11.666 3.32659 11.666 2.82274V1.84959C11.666 1.34574 12.0989 0.937256 12.6328 0.937256C13.1667 0.937256 13.5996 1.34574 13.5996 1.84959V2.82274C13.5996 3.32659 13.1667 3.73507 12.6328 3.73507Z" />
                      <path d="M21.4808 5.19089C21.1033 5.54719 20.4911 5.54725 20.1135 5.19089C19.736 4.8346 19.736 4.25697 20.1135 3.90067L20.8427 3.21253C21.2202 2.85624 21.8324 2.85624 22.21 3.21253C22.5876 3.56883 22.5876 4.14646 22.21 4.50275L21.4808 5.19089Z" />
                      <path d="M24.2988 11.946H23.2676C22.7336 11.946 22.3008 11.5375 22.3008 11.0337C22.3008 10.5298 22.7336 10.1213 23.2676 10.1213H24.2988C24.8328 10.1213 25.2656 10.5298 25.2656 11.0337C25.2656 11.5375 24.8328 11.946 24.2988 11.946Z" />
                      <path d="M3.78464 5.19083L3.05548 4.50275C2.67791 4.14645 2.67791 3.56883 3.05548 3.21253C3.43298 2.85624 4.04516 2.85624 4.42273 3.21253L5.15188 3.90061C5.52945 4.25691 5.52945 4.83453 5.15188 5.19083C4.77438 5.54706 4.16214 5.54712 3.78464 5.19083Z" />
                      <path d="M0.966797 10.1213H1.99805C2.53198 10.1213 2.96484 10.5298 2.96484 11.0337C2.96484 11.5375 2.53198 11.946 1.99805 11.946H0.966797C0.432867 11.946 0 11.5375 0 11.0337C0 10.5298 0.432867 10.1213 0.966797 10.1213Z" />
                      <path d="M5.47852 25.6309H12.1363C11.6567 24.4323 12.0934 23.0522 13.261 22.3072L19.2456 18.4888C19.4055 18.3867 19.5941 18.3322 19.7871 18.3322H25.2012V17.4199C25.2012 16.916 25.634 16.5076 26.168 16.5076H32.0332C32.5671 16.5076 33 16.916 33 17.4199V29.2802C33 29.784 32.5671 30.1925 32.0332 30.1925H26.168C25.634 30.1925 25.2012 29.784 25.2012 29.2802V28.3679H23.7974L17.5413 31.0245C17.4165 31.0775 17.2813 31.1048 17.1445 31.1048H5.47852C3.87924 31.1048 2.57812 29.877 2.57812 28.3679C2.57812 26.8587 3.87924 25.6309 5.47852 25.6309ZM27.1348 28.3679H31.0664V18.3322H27.1348V28.3679ZM5.47852 29.2802H16.937L23.1931 26.6235C23.3179 26.5706 23.4531 26.5432 23.5898 26.5432H25.2012V20.1569H20.0833L14.3441 23.8187C13.8743 24.1185 13.7401 24.7098 14.0385 25.1648C14.2343 25.4635 14.5745 25.6309 14.8886 25.6309C14.9203 25.6309 14.9516 25.6325 14.9826 25.6354C15.1444 25.628 15.307 25.5856 15.4577 25.504L18.5972 23.8054C19.0604 23.5548 19.6512 23.706 19.9168 24.1431C20.1823 24.5802 20.0221 25.1378 19.5589 25.3884L16.4194 27.0869C15.9473 27.3423 15.4345 27.4642 14.929 27.4642C14.929 27.4642 15.3796 27.4555 5.47852 27.4555C4.94542 27.4555 4.51172 27.8647 4.51172 28.3678C4.51172 28.8708 4.94542 29.2802 5.47852 29.2802Z" />
                    </g>
                    <defs>
                      <clipPath id="clip0_51_376">
                        <rect
                          width={33}
                          height="31.1408"
                          fill="white"
                          transform="matrix(-1 0 0 1 33 0.450684)"
                        />
                      </clipPath>
                    </defs>
                  </svg>{" "}
                  development programs <i className="fa-solid fa-arrow-right" />
                </button>
              </div>
            </div>

            {/* Tab Content Column */}
            <div className="col-12 col-lg-7 col-xl-7">
              <div className="fc-profit__content">
                {/* Tab 1 Content: Feeding the Helpless */}
                <TabContent
                  isActive={active === 1}
                  id={tabData[0].id}
                  title={tabData[0].title}
                  description={tabData[0].description}
                  imgSrc={tabData[0].imgSrc}
                />

                {/* Tab 2 Content: Healing the Vulnerable */}
                <TabContent
                  isActive={active === 2}
                  id={tabData[1].id}
                  title={tabData[1].title}
                  description={tabData[1].description}
                  imgSrc={tabData[1].imgSrc}
                />

                {/* Tab 3 Content: Shelter and Care */}
                <TabContent
                  isActive={active === 3}
                  id={tabData[2].id}
                  title={tabData[2].title}
                  description={tabData[2].description}
                  imgSrc={tabData[2].imgSrc}
                />
              </div>
            </div>
          </div>
        </div>

        {/* Decorative Shapes */}
        <div
          className="shape-left"
          data-aos="fade-right"
          data-aos-duration={1000}
          data-aos-delay={300}
        >
          <img
            src="assets/images/profit-vector.png"
            alt="Image_inner"
            className="base-img"
          />
        </div>
        <div
          className="shape-right"
          data-aos="zoom-in"
          data-aos-duration={1000}
          data-aos-delay={300}
        >
          <img src="assets/images/fc-eleven.png" alt="Image_inner" />
        </div>
      </section>

      {/* Removed: ModalVideo component */}
    </>
  );
};

export default ProfitOne;
